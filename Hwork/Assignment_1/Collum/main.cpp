...
void print(const int [][COLMAX],int,int);//Either table can be printed
...
int main(int argc, char** argv) {
    ...
    print(array,row,col);//setw(10)
    
    //Output the augmented array
    cout<<endl<<"The Augmented Array"<<endl;
    print(augAry,row+1,col+1);//setw(10)
        ...
        for(int j=0;j<col;j++){
            ...
            if(augAry[i][j]==0){
                for(){
                    ...
                    temp=array[i-k][k]+array[k][j-(col)];
                    cout<< temp;
                }
                augAry[i][j]=temp;
            }
...
void print(const int array[][COLMAX],int row,int col){
